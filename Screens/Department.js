...
export default function AddDepartmentScreen({ navigation }) {
  ...
  const fetchDepartmentContent = async (departmentId) => {
    try {
      const response = await axios.get(`http://192.168.0.238:5000/grades/${departmentId}`);
  ...
  const handleDepartmentPress = async (department) => {
    try {
      const content = await fetchDepartmentContent(department.dept_ID);
      navigation.navigate('Grades', { department });
    } catch (error) {
  ...
  const filteredDepartments = departments.filter(department =>
    department.dept_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    department.dept_info.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <ScrollView>
        ...
        <View style={styles.departmentList}>
          {filteredDepartments.map((department) => (
            ...
            >
              <Text style={styles.departmentText}>{department.dept_name}</Text>
              <Text style={styles.departmentInfo}>{department.dept_info}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
      ...
      >
        <View style={styles.modalContainer}>
          <Animated.View style={[styles.modalView, modalStyle]}>
            ...
            <Text style={styles.modalTitle}>Create A New Department!</Text>
            <TextInput
              style={styles.input}
